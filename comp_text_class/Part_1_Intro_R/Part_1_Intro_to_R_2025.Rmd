...
```{r}
#count the number of rows in a spreadsheet 
nrow(MediaBucks)
```
...
New_York_Times <- subset(MediaBucks, grepl("?NewYorkTimes", COMPANY)) |> 
  clean_names()
...
# The highest paid position is Executive News Editor, who is black and gets paid 350000, does not list the gender. 
...
business_companies <- subset(MediaBucks, grepl("?Business", COMPANY)) |> 
  clean_names() |> 
...
summarise(total_salary = sum(salary)) |> 
  print()
            ...
            #Which one is higher? Bloomberg's total salary is 1155540 and the business is 765818, so Bloomberg is higher. ]]]]]]
