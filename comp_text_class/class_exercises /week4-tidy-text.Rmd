...
```{r}
# A tibble is a modern version of a data frame in R, designed for better readability, performance, and usability. It comes from the {tibble} or {tidyverse} package.
...
```{r}
#The unnest_tokens() function from the {tidytext} package is used to split (tokenize) text into individual words, sentences, or other units while keeping it in a tidy format. This is useful in text mining, natural language processing (NLP), and sentiment analysis.
#can call word anything 
text_df %>%
...
```{r}
#showcasing the book titles in the dataframe 
original_books |> 
  count(book)
```
...
```{r}
#it strips out stop words for the dataset 
data(stop_words)
...
```{r}
#sort= TRUE is showing the word frequency descending, same thing as doing arrange(Desc) or arrange(desc(n))
tidy_books %>%
...
```{r}
#making a bar graph of the most frequent words that appeared more thant 600 times in the dataframe, arrranging them in descending order, making a new column with the word and the number of times its appeared within the books, labs(y=NULL), ensure that the y- axis doesn't have a name 
library(ggplot2)
...
```{r }
#splitting the sentences into words, and taking the stop(common) words away
tidy_bronte <- bronte %>%
  unnest_tokens(word, text) %>%
  anti_join(stop_words)
```
...
frequency <- bind_rows(mutate(tidy_bronte, author = "Brontë Sisters"),
 ...
 pivot_longer(`Brontë Sisters`:`H.G. Wells`:`Jane Austen`,
...
Tips/Tricks: 
-  tokenizing text- the process of splitting phrases, sentences, words, and chracters 
(unnest tokens) - is the tidytext function that allow you to tokenize 
